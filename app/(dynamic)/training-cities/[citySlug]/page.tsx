...
export async function generateMetadata({
  ...
  try {
    ...
    return {
      title:
        city.meta_title ||
        `${city.title} Training Courses | الدول التدريبية العالمية`,
      description:
      ...
      keywords:
        city.meta_keywords ||
        `${city.title}, training courses ${
          city.title
        }, professional training, الدول التدريبية العالمية, skill development, ${city.title.toLowerCase()} courses`,
      ...
      openGraph: {
        ...
        title:
          city.meta_title || city.h1 || `Training Courses in ${city.title}`,
        description:
          city.meta_description || city.description?.replace(/<[^>]*>/g, ""),
        images: [
          {
            url:
              city.meta_image || city.image || "/assets/images/hero-city.webp",
            width: 1200,
            height: 630,
            alt:
              city.image_alt ||
              city.image_title ||
              `Training courses in ${city.title}`,
            type: "image/jpeg",
          },
        ],
      },
    ...
    return {
      title:
        "Training Courses | الدول التدريبية العالمية Professional Development",
      description:
        "Professional training courses by الدول التدريبية العالمية. Enhance your skills with our expert-led programs designed for career advancement and professional growth.",
      keywords:
        "professional training, courses, الدول التدريبية العالمية, skill development, career advancement",
      authors: [{ name: "الدول التدريبية العالمية" }],
      creator: "الدول التدريبية العالمية",
      publisher: "الدول التدريبية العالمية",
      robots: {
...
export default async function Page({ params, searchParams }: CityPageProps) {
  ...
  const citySchema = {
    ...
    description:
      city.description?.replace(/<[^>]*>/g, "") ||
      `Professional training courses in ${city.title}`,
    ...
    <>
      ...
      <header>
        <HeroBanner
          ...
          description={
            city.description || `ابحث عن دورات تدريبية مهنية في ${city.title}`
          }
